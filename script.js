...
async function renderSajuResult(name, yP, mP, dP, tP) {
    ...
    const prompt = `당신은 20년 경력의 친절하고 통찰력 있는 명리학 최고 권위자입니다.
다음은 내담자의 사주 정보입니다.
- 이름: ${name}
- 사주팔자(천간/지지): 년주(${yP}), 월주(${mP}), 일주(${dP}), 시주(${tP})
- 오행 분포: 목(${counts.wood}), 화(${counts.fire}), 토(${counts.earth}), 금(${counts.metal}), 수(${counts.water})

이 정보를 바탕으로 아래의 카테고리를 포함하여 매우 상세하고 친절하게 사주를 풀이해주세요. 
전문 용어는 알기 쉽게 비유를 들어 설명해주고, 희망적이고 실질적인 조언이 되는 방향으로 작성해주세요. (총 분량은 최소 800자 이상으로 길게)
1. 🌟 총평 (타고난 우주의 기운과 본질적 성향)
2. 💰 재물운 (돈을 모으는 방법, 주의할 점, 유리한 투자 방향)
3. 💼 직장/사업운 (가장 잘 맞는 직업적 적성, 사회생활 조언)
4. ❤️ 애정/대인관계운 (연애 성향, 귀인을 만나는 방법)
5. 🩺 건강운 (오행의 불균형을 통한 취약 질환 및 건강 관리법)
